#!/usr/bin/env python3
"""
Distribution Predictor using Block Sampling Features - Full Dataset Version.
"""
...
def main():
    ...
    # Use all data (not unique) for better training
    # The block features are deterministic for same data, so duplicates are fine
    print(f"Using all samples (not deduplicated)")

    # Split data stratified by distribution
    ...
    if len(results) >= 2:
        orig = results[0]
        combined = results[-1]
        improvement = (combined['accuracy'] - orig['accuracy']) / orig['accuracy'] * 100
        print(f"\nImprovement from block features: {improvement:+.1f}%")
